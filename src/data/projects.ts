...
Built with Clean Architecture and CQRS on the backend (ASP.NET Core 10 + MediatR), and a React 19 SPA on the frontend. Authentication is handled by Keycloak with mandatory TOTP 2FA. Documents are stored in MinIO/S3 with presigned URLs. All branding — company name, logo, colors, and contact info — is configurable via the UI without redeployment.`,
    ...
    techStack: [
      { category: "Backend",        items: ["ASP.NET Core 10", "C# 13", "Clean Architecture", "CQRS (MediatR)", "FluentValidation", "EF Core 9"] },
      { category: "Database",       items: ["MariaDB", "MySQL"] },
      { category: "Auth & Storage", items: ["Keycloak (OIDC, TOTP 2FA)", "MinIO / S3"] },
      { category: "Frontend",       items: ["React 19", "TypeScript", "Vite", "Redux Toolkit (RTK Query)", "Tailwind CSS 4"] },
      { category: "Infrastructure", items: ["Docker", "Docker Compose", "Serilog", "MailKit (SMTP)", "Swagger UI"] },
    ],
    status: "completed",
    visibility: "public",
    featured: true,
...
All data is stored locally on your device (SQLite via Drift) and syncs to a spreadsheet in your own Google Drive. The developer has absolutely zero access to your financial data — no backend, no server, no analytics. Uses Google Sheets API purely as a zero-cost, user-owned cloud storage layer.`,
    ...
    techStack: [
      { category: "Framework",        items: ["Flutter 3.29", "Dart 3.7"] },
      { category: "State & Database", items: ["GetX", "Drift (SQLite)"] },
      { category: "Auth & Sync",      items: ["Google Sign-In (OAuth 2.0)", "Google Sheets API v4", "Google Drive API"] },
      { category: "Charts & OCR",     items: ["FL Chart", "Google ML Kit (on-device OCR)"] },
      { category: "Security",         items: ["local_auth (Biometric + PIN)", "flutter_secure_storage", "WorkManager"] },
    ],
    status: "completed",
    visibility: "public",
    featured: true,
